...
public class ModulesFragment extends ListFragment implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        String nameModule = c.getString(c.getColumnIndex(Contract.ModuleColumns.COLUMN_MODULE_NAAM));
        //voorlopig --> beter: nieuw fragment oproepen
        List<Docent> docenten = ModuleProvider.getInstance().getDocentenModule(nameModule);
        String temp = "";
        for(Docent docent: docenten) temp += docent.toString() + " - ";
        if (temp.length() >= 2) temp = temp.substring(0,temp.lastIndexOf(" - "));
